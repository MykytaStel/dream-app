...
export function getMonthlyReportMonths(
  dreams: Dream[],
  locale: string,
): MonthlyReportMonth[] {
...
export function getMonthlyReportData(
  ...
  const monthDreams = dreams.filter(dream => {
    ...
    return (
      date.getFullYear() === parsed.year && date.getMonth() + 1 === parsed.month
    );
  });

  const voiceCount = monthDreams.filter(dream =>
  ...
  const contextCount = monthDreams.filter(dream =>
    Boolean(dream.sleepContext),
  ).length;
  const totalWords = monthDreams.reduce(
    (sum, dream) => sum + countDreamWords(dream.text),
    0,
  );
  const recurringThemes = getRecurringReflectionSignals(monthDreams, {
    limit: 1,
  });
  ...
  return {
    month: {
      key: fallbackMonth,
      label: new Date(parsed.year, parsed.month - 1, 1).toLocaleDateString(
        'en-US',
        {
          month: 'short',
        },
    ...
    totalWords,
    averageWords: monthDreams.length
      ? Math.round(totalWords / monthDreams.length)
      : 0,
    voiceCount,
    ...
    topTheme: recurringThemes[0]
      ? {
          label: recurringThemes[0].label,
          dreamCount: recurringThemes[0].dreamCount,
        }
      : undefined,
    topSymbol: recurringSymbols[0]
      ? {
          label: recurringSymbols[0].label,
          dreamCount: recurringSymbols[0].dreamCount,
        }
      : undefined,
    topWord: recurringWords[0]
      ? {
          label: recurringWords[0].label,
          dreamCount: recurringWords[0].dreamCount,
        }
      : undefined,
